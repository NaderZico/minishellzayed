...
void	finalize_middle_command(t_pipe *pipe, t_cmds *cmds, t_vars *v)
{
	close(pipe->e_fd);
	closing_fds(pipe);
	if (builtins_pipes(cmds, pipe, pipe->pipe_fds[0][1], v->cmd_i) == 0)
		free_and_exit(pipe, cmds);
	pipe->cmd_exec = check_command_existence(cmds[v->cmd_i].cmd[0],
			pipe->m_path);
	if (!pipe->cmd_exec)
		child_exit(cmds, v->cmd_i, pipe, 0);
	if (execve(pipe->cmd_exec, cmds[v->cmd_i].cmd, NULL) < 0)
	{
